...
job_ids = []
get_job_ids = "https://boards.greenhouse.io/embed/job_board?for=buzzfeed&b=https://www.buzzfeed.com/about/jobs"
got = requests.get(get_job_ids)
job_ids = re.findall('jid=([0-9]+)',got.content.decode('utf-8'))
print(job_ids)

#board token is company name, job id for buzzfeed is 6 digit sequence
for job_id in job_ids:
    uri = 'https://api.greenhouse.io/v1/boards/buzzfeed/jobs/' + job_id
    r = requests.get(uri)
    ...
    try:
        if json.loads(r.content)["offices"][0]["id"] not in ignore_offices:
            ...
            all_descriptions = all_descriptions + ' ' + descrip
    except:
        print('Error with listing no. ' + job_id)
        continue
